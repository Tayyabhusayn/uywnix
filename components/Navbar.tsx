        ...
        <div className="container mx-auto px-6">
          <div className="flex items-center justify-between h-20">
            ...
            <div className="hidden lg:flex items-center gap-1">
              {navLinks.map((link) => (
                <Link
                  key={link.href}
                  href={link.href}
                  className="relative px-4 py-2 text-sm font-semibold text-slate-500 hover:text-slate-900 transition-colors group"
                >
                  {link.label}
            ...
            <div className="hidden lg:flex items-center gap-3">
              <Link
                href="/audit"
                className="group relative inline-flex items-center gap-2 bg-slate-900 text-white px-6 py-2.5 rounded-full text-sm font-semibold overflow-hidden transition-all duration-300 hover:shadow-lg hover:shadow-slate-900/20"
              >
            ...
            <motion.button
              whileTap={{ scale: 0.95 }}
              className="lg:hidden p-2 text-slate-900 relative z-[110]"
      ...
      <AnimatePresence>
        {isMenuOpen && (
          <motion.div
            ...
            exit={{ opacity: 0 }}
            transition={{ duration: 0.3 }}
            className="fixed inset-0 bg-white z-[100] lg:hidden"
          >
              ...
              <div className="flex flex-col gap-4">
                {navLinks.map((link, i) => (
                  ...
                  >
                    <Link
                      href={link.href}
                      className="text-4xl font-black text-slate-900 tracking-tight hover:text-violet-600 transition-colors"
