      ...
      <main className="flex-1 container mx-auto px-4 py-12 md:py-24 max-w-5xl">
        ...
        <div className="grid gap-8">
          {allPosts.map((article) => (
            <Link key={article.id} href={`/newsroom/${article.id}`} className="group block bg-white/5 p-8 rounded-3xl border border-white/10 shadow-sm hover:shadow-xl hover:border-violet-400/40 transition-all duration-300 hover:-translate-y-1">
              <div className="flex flex-col md:flex-row gap-6 md:items-start">
                ...
                <div className="shrink-0 mt-4 md:mt-0">
                  <span className="inline-flex items-center justify-center w-10 h-10 rounded-full bg-white/5 text-slate-400 group-hover:bg-violet-500/10 group-hover:text-violet-300 transition-colors">
